...
struct xyz
{
  ...
  {
    return sqrt((pow((x - p.x), 2) + pow((x - p.x), 2) + pow((x - p.x), 2)));
  }
...
bool sanity_check_npz_file(std::map<std::string, cnpy::NpyArray>& a,
                 ...
                 std::vector<float>& distances)
{
  ...
  for (size_t j = offset; j < offset + Num_dist; j++) {
    ...
    if (j - offset >= distances.size()) {
      LBANN_ERROR("j-offset >= data.size(); j-offset: ",
                  j - offset,
                  " data.size: ",
                  data.size());
    }

  ...
  try {
    ...
    for (size_t j = rank; j < filenames.size(); j += np) {
      ...
      if (n_beads != Num_beads) {
        LBANN_ERROR("n_beads != Num_beads; n_beads: ",
                    n_beads,
                    " Num_beads: ",
                    Num_beads);
      }
      ...
      ++nn;
      if (!rank) {
        std::cerr << "approx " << (nn * np) << " files of " << filenames.size()
                  << " processed\n";
      }
    }
    ...
    MPI_Reduce(&v_minus_mean_squared,
               &all_minus_mean_squared,
               1,
               MPI_DOUBLE,
               MPI_SUM,
               0,
...
void read_file(const std::string& filename, std::vector<float>& data)
{
  ...
  if (static_cast<size_t>(in.gcount()) != n) {
    LBANN_ERROR("in.gcount() != n (gcount: ",
                in.gcount(),
                "; n: ",
                n,
                ") for file: ",

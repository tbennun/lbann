...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    if (argc == 1) {
      if (master) {
        std::cerr << "usage: " << argv[0]
                  << " --filelist=<string> --output_fn=<string>" << std::endl;
      }
      return EXIT_FAILURE;
    }
    ...
    for (size_t j = rank; j < filenames.size(); j += np) {
      ...
      double* data =
        reinterpret_cast<double*>(a["density_sig1"].data_holder->data());
      ...
      if (master) {
        std::cerr << "approx " << utils::commify(total_elts_per_channel * np)
    ...
    comm->trainer_allreduce(v_max.data(),
                            v_max.size(),
                            f_max.data(),
                            El::mpi::MAX);
    comm->trainer_allreduce(v_min.data(),
                            v_min.size(),
                            f_min.data(),
                            El::mpi::MIN);
    comm->trainer_allreduce(v_mean.data(),
                            v_mean.size(),
                            f_mean.data(),
                            El::mpi::SUM);
    ...
    for (size_t j = rank; j < filenames.size(); j += np) {
      ...
      for (size_t i = 0; i < n_elts; i++) {
        ...
        // std::cout << "vv: " << vv << " f_mean[s]" << f_mean[s] << "  (vv -
        // f_mean[s])*(vv - f_mean[s]): " << ((vv - f_mean[s])*(vv - f_mean[s]))
        // << " v_minus_mean_squared[s]: " << v_minus_mean_squared[s] <<
        // std::endl;
        ++s;
        if (s == 14) {
          s = 0;
    ...
    comm->trainer_allreduce(v_minus_mean_squared.data(),
                            v_minus_mean_squared.size(),
                            f_minus_mean_squared.data(),
                            El::mpi::SUM);
    if (master)
    ...
    for (size_t j = 0; j < f_minus_mean_squared.size(); j++) {
      ...
      if (j == 0) {
        std::cout << "f_minus_mean_squared[j]: " << f_minus_mean_squared[j]
                  << std::endl;
        std::cout << "count: " << n3 << "\n / count: " << v3 << std::endl;
      }
      f_std_dev[j] = sqrt(v3);
    ...
    if (master) {
      std::cerr << "\nactual num samples processed: " << utils::commify(n3)
                << std::endl;
    ...
    if (master)
      std::cout << "num samples: " << n3 << std::endl;
  }

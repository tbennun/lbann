...
void GemmUseCPU() { gemm_cpu_gpu_switch = 'c'; }
          ...
          Matrix<T>& C)
{
  ...
  if (k != 0) {
    ...
    if (gemm_cpu_gpu_switch == 'g' && m >= min_M && n >= min_N && k >= min_K) {
      cublas::Gemm(transA,
                   transB,
                   m,
                   n,
               ...
               AbstractDistMatrix<T>& C)
{
  ...
  EL_DEBUG_ONLY(
    if (orientA == NORMAL && orientB == NORMAL) {
      if (A.ColDist() != C.ColDist() || A.RowDist() != B.ColDist() ||
          B.RowDist() != C.RowDist())
        LogicError("Tried to form C[",
                   C.ColDist(),
                   ",",
                   C.RowDist(),
                   "] := "
      ...
      if (A.Height() != C.Height() || A.Width() != B.Height() ||
          B.Width() != C.Width())
        LogicError("Nonconformal LocalGemmNN:\n",
                   DimsString(A, "A"),
                   "\n",
                   DimsString(B, "B"),
                   "\n",
      ...
      if (A.ColDist() != C.ColDist() || A.RowDist() != B.RowDist() ||
          B.ColDist() != C.RowDist())
        LogicError("Tried to form C[",
                   C.ColDist(),
                   ",",
      ...
      if (A.Height() != C.Height() || A.Width() != B.Width() ||
          B.Height() != C.Width())
        LogicError("Nonconformal LocalGemmNT:\n",
                   DimsString(A, "A"),
                   "\n",
                   DimsString(B, "B"),
                   "\n",
      ...
      if (A.RowDist() != C.ColDist() || A.ColDist() != B.ColDist() ||
          B.RowDist() != C.RowDist())
        LogicError("Tried to form C[",
                   C.ColDist(),
                   ",",
      ...
      if (A.Width() != C.Height() || A.Height() != B.Height() ||
          B.Width() != C.Width())
        LogicError("Nonconformal LocalGemmTN:\n",
                   DimsString(A, "A"),
                   "\n",
                   DimsString(B, "B"),
                   "\n",
      ...
      if (A.RowDist() != C.ColDist() || A.ColDist() != B.RowDist() ||
          B.ColDist() != C.RowDist())
        LogicError("Tried to form C[",
                   C.ColDist(),
                   ",",
      ...
      if (A.Width() != C.Height() || A.Height() != B.Width() ||
          B.Height() != C.Width())
        LogicError("Nonconformal LocalGemmTT:\n",
                   DimsString(A, "A"),
                   "\n",
                   DimsString(B, "B"),
                   "\n",
...
#define PROTO(T)                                                               \
  template void Gemm(Orientation orientA,                                      \
                     Orientation orientB,                                      \
                     T alpha,                                                  \
                     const Matrix<T>& A,                                       \
                     const Matrix<T>& B,                                       \
  ...
  template void Gemm(Orientation orientA,                                      \
                     ...
                     AbstractDistMatrix<T>& C,                                 \
                     GemmAlgorithm alg);                                       \
  template void LocalGemm(Orientation orientA,                                 \
                          Orientation orientB,                                 \
                          T alpha,                                             \
                          const AbstractDistMatrix<T>& A,                      \
                          const AbstractDistMatrix<T>& B,                      \

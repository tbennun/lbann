...
void EL_BLAS(sgemm)(const char* transA,
                    const char* transB,
                    const BlasInt* m,
                    const BlasInt* n,
                    const BlasInt* k,
                    ...
                    float* C,
                    const BlasInt* CLDim);
void EL_BLAS(dgemm)(const char* transA,
                    const char* transB,
                    const BlasInt* m,
                    const BlasInt* n,
                    const BlasInt* k,
                    ...
                    double* C,
                    const BlasInt* CLDim);
void EL_BLAS(cgemm)(const char* transA,
                    const char* transB,
                    const BlasInt* m,
                    const BlasInt* n,
                    const BlasInt* k,
                    ...
                    scomplex* C,
                    const BlasInt* CLDim);
void EL_BLAS(zgemm)(const char* transA,
                    const char* transB,
                    const BlasInt* m,
                    const BlasInt* n,
                    const BlasInt* k,
...
template void Gemm(char transA,
                   char transB,
                   BlasInt m,
                   BlasInt n,
                   BlasInt k,
...
template void Gemm(char transA,
                   ...
                   const Complex<DoubleDouble>* A,
                   BlasInt ALDim,
                   const Complex<DoubleDouble>* B,
                   BlasInt BLDim,
...
template void Gemm(char transA,
                   ...
                   const Complex<QuadDouble>* A,
                   BlasInt ALDim,
                   const Complex<QuadDouble>* B,
                   BlasInt BLDim,
...
template void Gemm(char transA,
                   ...
                   const Complex<Quad>* A,
                   BlasInt ALDim,
                   const Complex<Quad>* B,
                   BlasInt BLDim,
...
template void Gemm(char transA,
                   ...
                   const Complex<BigFloat>* A,
                   BlasInt ALDim,
                   const Complex<BigFloat>* B,
                   BlasInt BLDim,
...
#if USE_CUB
  CubDebugExit(
    g_allocator.DeviceAllocate((void**)&devA,
          ...
          BlasInt CLDim)
{
  ...
  cublasStatus stat;
  stat =
    cublasAlloc(sizeA + sizeB + sizeC, sizeof(cuDoubleComplex), (void**)&devA);
  if (stat != CUBLAS_STATUS_SUCCESS) {
    RuntimeError("Alloc A,B,C error\n");
  }
  ...
  // copy matrix A, B and C to device
  stat =
    cublasSetMatrix(rowA, colA, sizeof(cuDoubleComplex), A, ALDim, devA, rowA);
  if (stat != CUBLAS_STATUS_SUCCESS) {
    RuntimeError("SetMatrix A error\n");
  }

  stat =
    cublasSetMatrix(rowB, colB, sizeof(cuDoubleComplex), B, BLDim, devB, rowB);
  if (stat != CUBLAS_STATUS_SUCCESS) {
    RuntimeError("SetMatrix B error\n");
  }

  if (beta.real() != 0.0 || beta.imag() != 0.0) {
    stat = cublasSetMatrix(rowC,
                           colC,
  ...
  // copy matrix C to host
  stat =
    cublasGetMatrix(rowC, colC, sizeof(cuDoubleComplex), devC, rowC, C, CLDim);
  if (stat != CUBLAS_STATUS_SUCCESS) {
    RuntimeError("GetMatrix C error\n");
  }

...
int main(int argc, char** argv)
{
  ...
  if (master)
    cerr << "opening output files\n";
  sprintf(b, "%s/TMP_%d", base_dir.c_str(), rank);
  ofstream out(b);
  if (!out)
    LBANN_ERROR("can't open file for writing");
  ...
  for (size_t j = rank; j < filenames.size(); j += np) {
    ...
    for (size_t h = 0; h < cnames.size(); h++) {
      ...
      if (success == 1) {
        ...
        // however, the top-level fields, e.g,
        // "274e5a16-7c3a-11e9-90fd-0894ef80059f," are unique, at least for the
        // current set of hydra bricks, so for now I'm using that field as the
        // sample_id. This has the advantage that the sample_ids are at the top
        // level, as they are for JAG samples
        out << cnames[h] << " ";
  ...
  if (master) {
    ...
    s3.clear();
    s3.str("");
    s3 << "chmod 660 " << base_dir << "/id_mapping.txt; chgrp brain "
       << base_dir << "/id_mapping.txt";

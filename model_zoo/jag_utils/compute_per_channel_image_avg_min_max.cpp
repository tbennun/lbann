...
int main(int argc, char* argv[])
{
  ...
  try {
      ...
      for (size_t i = 0; i < cnames.size(); i++) {
        ...
        if (success == 1) {
          ...
          try {
            ...
            for (int channel = 0; channel < 4; channel++) {
              for (size_t hh = channel; hh < image_size; hh += 4) {
                float val = emi[hh];
                if (val < v_min[channel])
                  v_min[channel] = val;
                if (val > v_max[channel])
                  v_max[channel] = val;
                v_sum[channel] += val;
            ...
            std::cerr << rank << " :: "
                      << "exception reading image: (0.0, 0.0) for sample: "
                      << cnames[i] << " which is " << i << " of " << cnames[i]
                      << "; " << files[j] << "\n";
            continue;
          }
          ...
          try {
            ...
            std::cerr << rank << " :: "
                      << "exception reading image: (90.0, 0.0) for sample: "
                      << cnames[i] << " which is " << i << " of " << cnames[i]
                      << "; " << files[j] << "\n";
            continue;
          }
          ...
          try {
            ...
            std::cerr << rank << " :: "
                      << "exception reading image: (90.0, 78.0) for sample: "
                      << cnames[i] << " which is " << i << " of " << cnames[i]
                      << "; " << files[j] << "\n";
            continue;
          }
    ...
    MPI_Reduce(v_sum.data(),
               ...
               0,
               MPI_COMM_WORLD);
    MPI_Reduce(v_num_pixels.data(),
               global_v_num_pixels.data(),
               4,
               MPI_LONG,
               MPI_SUM,
               0,
    ...
    if (master) {
      for (int j = 0; j < 4; j++) {
        std::cout << global_v_min[j] << " " << global_v_max[j] << " "
                  << global_v_avg[j] << " " << global_v_sum[j] << " "
                  << global_v_num_pixels[j] << "\n";
      }
    }
  }

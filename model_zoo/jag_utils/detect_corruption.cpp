...
void print_errs(world_comm_ptr& comm,
                int np,
                int rank,
                std::ostringstream& s,
                const char* msg);
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    if (arg_parser.get<std::string>(LBANN_OPTION_FILELIST) == "") {
      if (master) {
        throw lbann_exception(
          ...
          " :: usage: " + argv[0] +
          " --filelist=<string> \nwhere: 'filelist' is a file that contains "
          "the fully qualified filenames of the conduit *'bundle' files that "
          "are to be inspected.\nfunction: attemptsto detect and report "
          "currupt files and/or samples within those files.");
      }
    }
    ...
    for (size_t j = rank; j < filenames.size(); j += np) {
      h += 1;
      if (h % 1 == 0 && master)
        std::cerr << "P_0 has processed " << h << " files\n";
    ...
    print_errs(comm,
               ...
               rank,
               open_err,
               "failed to open these files (if any):");
    print_errs(comm,
               np,
               rank,
               children_err,
               "failed to read children from these files (if any):");
    print_errs(comm,
               np,
               rank,
               success_flag_err,
               "failed to read success flag for these samples (if any):");
    print_errs(
      comm,
      np,
      rank,
      sample_err,
      "failed to read input or scalars or images for these samples (if any):");
  }

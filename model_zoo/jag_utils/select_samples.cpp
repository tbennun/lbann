...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    unordered_map<string, unordered_set<int>> index_map_keep;
    // maps a samole_id filename to the set of sample_ids that have not been
    // randomly selscted
    unordered_map<string, unordered_set<int>> index_map_exclude;
    std::unordered_map<std::string, std::string> filename_data;
    build_index_maps(index_map_keep,
                     index_map_exclude,
                     string_to_index,
                     filename_data);
    ...
    write_bar_files(index_map_exclude,
                    sample_mapping,
                    sample_mapping_v,
                    filename_data);
...
string help_msg()
{
  stringstream err;
  err
    << "usage: select_samples --index_fn=<string> --sample_mapping_fn=<string> "
       "--num_samples_per_list=<int> --num_lists --output_dir=<string> "
       "--output_base_name=<string> --random_seed=<int>\n\n";
  ...
  unordered_map<string, int>& string_to_index)
{
  ...
  cout << "  FINISHED reading sample mapping: num lines processed: " << n
       << "; time: " << tm2 << "\n";
}
  ...
  unordered_map<string, string>& filename_data)
{
  ...
  int samples_per_list =
    options::get()->get_int(LBANN_OPTION_NUM_SAMPLES_PER_LIST);
  ...
  const string index_fn =
    options::get()->get_string(LBANN_OPTION_INDEX_FN).c_str();
  ...
  if (line != "CONDUIT_HDF5_EXCLUSION") {
    LBANN_ERROR(
      "error: 1st line in index file must contain: CONDUIT_HDF5_EXCLUSION\n");
  }
  ...
  cout << "  FINISHED generating random indices; time: "
       << lbann::get_time() - tm2 << endl;
  ...
  while (!in.eof()) {
    ...
    ++num_files;
    if (num_files % 1000 == 0)
      cout << num_files / 1000 << "K input lines processed\n";
    stringstream s(line);
    s >> fn >> good >> bad;
    ...
    if (index_map_exclude[fn].size() != bad) {
      LBANN_ERROR("exclude.size(): ",
                  index_map_exclude[fn].size(),
                  " should be: ",
                  bad,
                  " but isn't\n");
    }
    ...
    for (int local_index = 0; local_index < total; local_index++) {
      if (index_map_exclude[fn].find(local_index) ==
          index_map_exclude[fn].end()) {
  ...
  cout << "FINISHED selecting samples based on random indices; time: "
       << lbann::get_time() - tm3 << endl;

  if (index_map_exclude.size() != index_map_keep.size()) {
    LBANN_ERROR("index_map_exclude.size() != index_map_keep.size()");
  }
  cout << "  FINISHED build_index_maps; time: " << lbann::get_time() - tm1
       << endl;
}
...
void sanity_test_request()
{
  ...
  if (num_samples > num_valid) {
    LBANN_ERROR("you requested a total of ",
                num_samples,
                " samples, but only ",
                num_valid,
                " are available");
  ...
  const std::unordered_map<std::string, std::string>& filename_data)
{
  ...
  for (auto& t : subsets[n]) {
    ...
    std::unordered_map<std::string, std::string>::const_iterator t4 =
      filename_data.find(filename);
    ...
    if (included) {
      ...
      for (auto& t3 : include_me) {
        if (sample_mapping_v.find(fn2) == sample_mapping_v.end()) {
          LBANN_ERROR("failed to find the key: ",
                      fn2,
                      " in sample_mapping_v map");
        }
        unordered_map<string, vector<string>>::const_iterator t5 =
          sample_mapping_v.find(fn2);
...
void make_dir(char* cpath)
{
  ...
  int err =
    mkdir(cpath, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IXUSR | S_IXGRP);
  if (err) {
    free(cpath);
    LBANN_ERROR(
      "mkdir failed for \"",
      cpath,
      "\"; please create this directory yourself, then rerun this program");
    ...
    err = chmod(cpath,
                S_ISGID | S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IXUSR |
                  S_IXGRP);
    if (err) {
      cout << "   mkdir failed: " << strerror(errno) << endl;
    }
  ...
  const unordered_map<string, string>& filename_data)
{
  ...
  const string dir = options::get()->get_string(LBANN_OPTION_OUTPUT_DIR);
  const string base_fn =
    options::get()->get_string(LBANN_OPTION_OUTPUT_BASE_FN);
  stringstream s;

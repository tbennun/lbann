 ...
 */
image_list::image_list(const std::string data_path_file,
                       const bool write_cropped,
                       const mpi_states& ms)
{
  std::string buf;
...
bool image_list::read_path(const std::string& line,
 ...
 */
bool image_list::read_image_file_name(const std::string& line,
                                      std::vector<std::string>& container)
{
  std::stringstream sstr(line.c_str());
...
/**
 *  The root MPI rank loads a file into a buffer, then broadcast it to other
 * ranks
 */
void image_list::load_file(const std::string& file_name,
                           const mpi_states& ms,
                           std::string& buf)
{
...
/// Split the image file list among MPI ranks and have each rank put its portion
/// into a vector
...
void image_list::create_dirs(const mpi_states& ms,
                             const bool write_cropped) const
{
  if (ms.is_root()) {
    ...
    if (write_cropped) { // only create subdirectories when needed to write
                         // cropped images
      create_subdirs(ms);
    }
  }
...
/// Create the sub directories for each individual image under the root output
/// directory.
...
void image_list::split_list(const mpi_states& ms)
{
  if (m_image_list.size() < static_cast<size_t>(ms.get_num_ranks())) {
    ...
    if (ms.is_root()) {
      std::cerr
        << "Needlessly large number of ranks compared to the number of images."
        << std::endl;
    }
#else
    ms.abort(
  ...
  std::cout << "rank " << ms.get_my_rank() << " has images ["
            << num_per_rank[ms.get_my_rank()] << " - "
            << num_per_rank[ms.get_my_rank() + 1] << ')' << std::endl;
  ...
  std::vector<std::string> selected(
    m_image_list.begin() + num_per_rank[ms.get_my_rank()],
...
std::string
image_list::get_image_name_with_new_ext(const size_t i,
                                        const std::string new_ext) const
{
  std::string subdir;
  std::string basename;
  lbann::parse_path(m_image_list.at(i), subdir, basename);
  return (m_out_dir + subdir + lbann::get_basename_without_ext(basename) +
          new_ext);
}

/// Print out the path prameters read, and the number of images for this rank to
/// process.

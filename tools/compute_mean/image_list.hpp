...
protected:
   ...
   *  If cropped images are stored, they will be written to the same relative
   * path as they were in the input directory but relative to the root output
   * directory instead of the root data directory.
   */
...
protected:
  static bool read_path(const std::string& line,
                        std::vector<std::string>& container);
  static bool read_image_file_name(const std::string& line,
                                   std::vector<std::string>& container);
  static void load_file(const std::string& file_name,
                        const mpi_states& ms,
                        std::string& buf);
...
public:
  image_list(const std::string data_path_file,
             const bool write_cropped,
             const mpi_states& ms);

  /// Return the num of ranks that only have assigned data.
  unsigned int get_effective_num_ranks() const { return m_effective_num_ranks; }
  ...
  std::string get_root_data_path() const { return m_root_data_path; }
  /// Return the root output directory.
  std::string get_out_dir() const { return m_out_dir; }
  std::string get_image_name_with_new_ext(const size_t i,

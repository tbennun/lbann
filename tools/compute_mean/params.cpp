...
std::string params::show_help(std::string name)
{
  ...
  oss << "Usage: > " << name << " path_file w h [ r rw rh ] bsz save"
      << std::endl
      << "       > " << name << " path_file" << std::endl
      << std::endl
      << "  path_file: contains the paths to the root data directory, the "
         "image list file"
      << std::endl
      << "             and the output directory. The list file contains the "
         "relative path"
      << std::endl
      << "             of each image to the root directory." << std::endl
      << "             If this is the only argument, it will create the output "
      ...
      << std::endl
      << "    r: whether to randomize the crop position within the center "
         "region. (0|1)"
      << std::endl
      << "   rw: The width of the center region with respect to w after "
         "resizig the raw image."
      << std::endl
      << "   rh: The height of the center region with respect to h after "
         "resizing the raw image."
      << std::endl
      << "       Raw image will be resized to an image of size rw x rh around "
         "the center,"
      << std::endl
      << "       which covers area of the original image as much as possible "
         "while preseving"
      << std::endl
      << "       the aspect ratio of object in the image." << std::endl
      << "       When r, rw, and rh are omitted, it is assumed that r=0, rw=w, "
         "and rh=h."
      << std::endl
      << std::endl

...
{
  if (argc < 4) {
    cout << "Usage .... exec input_file output_file_basename num_partitions"
         << endl;
    exit(-1);
  }
  ...
  std::cout << "Original file shuffled and save as: " << output_file
            << std::endl;
  ofstream base_ofs(output_file.c_str());
  if (!base_ofs) {
    std::cout << "\n In write_file can't open file : " << output_file;
    exit(1);
  }
  for (auto& l0 : lines)
    base_ofs << l0.first << " " << l0.second << std::endl;
  ...
  for (int p = 0; p < num_partition; p++) {
    ...
    if (!ofs) {
      std::cout << "\n In write_file: can't open file : " << partition_file;
      exit(1);
    }
